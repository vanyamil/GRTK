...
namespace GRTK
{
	...
	public class BoundaryGraph : IEnumerable<BoundaryGraphNode>
	{
		...
		public void Unify()
		{
			List<UnorderedPair<BoundaryGraphNode>> edges = getEdgesAsUniquePairs();
			// Intersect each edge using Line2D and split the edges
			int i, j;
			for (i = 0; i < edges.Count; i++)
			{
				UnorderedPair<BoundaryGraphNode> edge1 = edges[i];
				...
				{
					UnorderedPair<BoundaryGraphNode> edge2 = edges[j];
					...
					if (line1.Intersect(line2, out intersection))
					{
						...
						edges.Add(new UnorderedPair<BoundaryGraphNode>(edge1.item1, intersectionNode));
						edges.Add(new UnorderedPair<BoundaryGraphNode>(edge1.item2, intersectionNode));
						edges.Add(new UnorderedPair<BoundaryGraphNode>(edge2.item1, intersectionNode));
						edges.Add(new UnorderedPair<BoundaryGraphNode>(edge2.item2, intersectionNode));
		...
		public List<UnorderedPair<BoundaryGraphNode>> getEdgesAsUniquePairs()
		{
			// Find each unique edge as a pair of nodes using hashset
			HashSet<UnorderedPair<BoundaryGraphNode>> edges = new HashSet<UnorderedPair<BoundaryGraphNode>>();
			foreach (BoundaryGraphNode node1 in this)
			{
				foreach (BoundaryGraphNode node2 in this)
				{
					...
					UnorderedPair<BoundaryGraphNode> temp = new UnorderedPair<BoundaryGraphNode>(node1, node2);
					edges.Add(temp);
				}
			}

			// Convert to list and return
			return new List<UnorderedPair<BoundaryGraphNode>>(edges);
		}
